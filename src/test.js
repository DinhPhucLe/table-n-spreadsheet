...
function SetHeading(table){
  useEffect(() => {
    ...
    tmp.setDataAtCell(1, 1, "GIÁ NHỎ NHẤT");
    tmp.setCellMeta(1, 1, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 4, "SUPERSHIP (nội bộ)");
    tmp.setCellMeta(1, 4, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 7, "SUPERSHIP (nội bộ)");
    tmp.setCellMeta(1, 7, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 10, "SUPERSHIP (ngoài vùng)");
    tmp.setCellMeta(1, 10, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 13, "Ninja Van");
    tmp.setCellMeta(1, 13, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 16, "BEST");
    tmp.setCellMeta(1, 16, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 17, "Shopee Express");
    tmp.setCellMeta(1, 17, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 20, "GHN");
    tmp.setCellMeta(1, 20, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 23, "Viettel Post");
    tmp.setCellMeta(1, 23, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 26, "GHTK");
    tmp.setCellMeta(1, 26, 'className', `htCenter htMiddle heading`);

    tmp.setDataAtCell(1, 31, "J&T (Liên hệ phòng HTĐT)");
    tmp.setCellMeta(1, 31, 'className', `htCenter htMiddle heading`);
    
    tmp.setDataAtCell(1, 34, "Snappy");
    tmp.setCellMeta(1, 34, 'className', `htCenter htMiddle heading`);
...
function SetTableData(table, data){
  useEffect(() => {
    ...
    for (let r = 0; r < 12; r++){
      for (let c = 0; c < 35; c++){
        tmp.setDataAtCell(r+4, c+1, data[r][c]);
        //tmp.updateSettings(r+4, c+1);
      }
    }
  }, []);
}
